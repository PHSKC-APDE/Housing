...
count_acute_f <- function(df,
  ...
  if (!missing(age_var)) {
    agex <- rlang::sym(paste0("age", year, age_var))
    print(agex)
    group_var <- append(group_var, agex)
    print(group_var)
  } 
  
  if (!missing(len_var)) {
    lengthx <- rlang::sym(paste0("age", year, len_var))
    group_var <- group_var + lengthx
    print(group_var)
  } 
  
  # Figure out which condition is being examined
