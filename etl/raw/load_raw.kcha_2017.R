#### CODE TO LOAD 2017 KING COUNTY HOUSING AUTHORITY DATA
...
load_raw.kcha_2017 <- function(conn = NULL,
                               to_schema = NULL,
                               to_table = NULL,
                               qa_schema = NULL,
                               qa_table = NULL,
  ...
  # BRING IN DATA ----
  kcha_p1_2017 <- fread(file = file.path(file_path, "kcha_panel_01_2017.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  kcha_p2_2017 <- fread(file = file.path(file_path, "kcha_panel_02_2017.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  kcha_p3_2017 <- fread(file = file.path(file_path, "kcha_panel_03_2017.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, 'raw.kcha_2017',
                                  'value', 'row_count', {nrow(kcha_p1_2017)},
                                  {Sys.time()}, NULL)",
                          .con = conn))
  ...
  kcha_2017_full <- list(kcha_p1_2017, kcha_p2_2017, kcha_p3_2017) %>%
  ...
  ## Field names ----
  # Fix up an inconsistent name in the 2017 data
  ...
  kcha_2017_full <- kcha_2017_full %>% 
    mutate(pha_source = "kcha2017",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(kcha_2017_full),
