...
load_raw.kcha_2018 <- function(conn = NULL,
                               to_schema = NULL,
                               to_table = NULL,
                               qa_schema = NULL,
                               qa_table = NULL,
  ...
  # BRING IN DATA ----
  kcha_p1_2018 <- fread(file = file.path(file_path, "kcha_panel_01_2018.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  kcha_p2_2018 <- fread(file = file.path(file_path, "kcha_panel_02_2018.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  kcha_p3_2018 <- fread(file = file.path(file_path, "kcha_panel_03_2018.csv"), 
                        na.strings = c("NA", "", "NULL", "N/A", "."), 
                        stringsAsFactors = F)
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, 'raw.kcha_2018',
