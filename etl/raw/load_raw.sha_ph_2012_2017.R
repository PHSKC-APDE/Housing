#### CODE TO LOAD 2012-2017 SEATTLE HOUSING AUTHORITY PUBLIC HOUSING DATA
...
load_raw.sha_ph_2012_2017 <- function(conn = NULL,
                                    to_schema = NULL,
                                    to_table = NULL,
                                    qa_schema = NULL,
                                    qa_table = NULL,
  ...
  # BRING IN DATA ----
  sha_ph_p1_2012_2017 <- fread(file = file.path(file_path, "sha_ph_2012_2017_panel_01_household.csv"), 
                             na.strings = c("NA", "", "NULL", "N/A", "."), 
                             stringsAsFactors = F)
  sha_ph_p2_2012_2017 <- fread(file = file.path(file_path, "sha_ph_2012_2017_panel_02_income_assets.csv"), 
                             na.strings = c("NA", "", "NULL", "N/A", "."), 
                             stringsAsFactors = F)
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                  'value', 'row_count', {nrow(sha_ph_p1_2012_2017)},
  ...
  sha_ph_2012_2017 <- list(sha_ph_p1_2012_2017 = sha_ph_p1_2012_2017, 
                           sha_ph_p2_2012_2017 = sha_ph_p2_2012_2017)
  ...
  sha_ph_2012_2017 <- sha_ph_2012_2017 %>%
    map(~ .x %>% 
  ...
  sha_ph_2012_2017 <- sha_ph_2012_2017 %>% map(~ .x %>% distinct())
  
  # Bring back to individual objects to make using data table easier
  list2env(sha_ph_2012_2017, envir = environment())
  rm(sha_ph_2012_2017)
  
  
  ## Panel 1 ----
  # Fix up date formats
  sha_ph_p1_2012_2017 <- sha_ph_p1_2012_2017 %>%
  ...
  sha_ph_p2_2012_2017_inc <- sha_ph_p2_2012_2017 %>%
  ...
  sha_ph_p2_2012_2017_asset <- sha_ph_p2_2012_2017 %>%
    group_by(incasset_id, inc_mbr_num) %>%
  ...
  sha_ph_2012_2017 <- left_join(sha_ph_p1_2012_2017, 
                             select(sha_ph_p2_2012_2017_inc, starts_with("inc")) %>% distinct(), 
                             by = c("incasset_id", "mbr_num" = "inc_mbr_num")) %>%
    left_join(., select(sha_ph_p2_2012_2017_inc, incasset_id, starts_with("hh_")) %>% distinct(),
              by = "incasset_id") %>%
    left_join(., select(sha_ph_p2_2012_2017_asset, starts_with("inc"), starts_with("asset")), 
              by = c("incasset_id", "mbr_num" = "inc_mbr_num")) %>%
    left_join(., select(sha_ph_p2_2012_2017_asset, incasset_id, starts_with("hh_")) %>% distinct(),
              by = "incasset_id")
  
  
  ## Join with portfolio data ----
  sha_ph_2012_2017 <- left_join(sha_ph_2012_2017, 
  ...
  sha_ph_2012_2017 <- sha_ph_2012_2017 %>% 
    mutate(pha_source = "sha2017_ph",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_ph_2012_2017),
