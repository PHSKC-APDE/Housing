#### CODE TO LOAD 2019 KING COUNTY HOUSING AUTHORITY DATA
...
load_raw_kcha_2019 <- function(conn = NULL,
                               to_schema = NULL,
                               to_table = NULL,
                               qa_schema = NULL,
                               qa_table = NULL,
  ...
  kcha_2019_full <- list(kcha_p1_2019, kcha_p2_2019, kcha_p3_2019) %>%
  ...
  kcha_2019_full <- kcha_2019_full %>% 
    mutate(pha_source = "kcha2019",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(kcha_2019_full),
