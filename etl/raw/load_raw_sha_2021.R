#### CODE TO LOAD 2021 SEATTLE HOUSING AUTHORITY PUBLIC HOUSING AND VOUCHER DATA
...
load_raw_sha_2021 <- function(conn = NULL,
                              to_schema = NULL,
                              to_table = NULL,
                              qa_schema = NULL,
                              qa_table = NULL,
  ...
  # BRING IN DATA ----
  sha_2021 <- fread(file = file.path(file_path, "sha_hcv_ph_2021.csv"), 
                    na.strings = c("NA", "", "NULL", "N/A", "."), 
                    stringsAsFactors = F, colClasses = 'character')
  rads::sql_clean(sha_2021)
  ...
  for(incvar in grep('income', names(sha_2021), value = T, ignore.case = T)){
    sha_2021[, paste0(incvar) := as.integer(get(gsub(",", "", incvar)))]
  }
  ...
  # QA CHECKS ----
    ...
    sha_2021[class(EFFECTIVE_DATE) != 'Date', EFFECTIVE_DATE := as.Date(as.integer(EFFECTIVE_DATE), origin = '1899-12-30')]
    dates <- sha_2021 %>%
    ...
    act_types <- sort(unique(sha_2021$CERT_TYPE[!is.na(sha_2021$CERT_TYPE)]))
    ...
    if (is.character(sha_2021$CERT_TYPE) & length(act_types[act_types %in% act_types_expected == F]) > 0) {
    ...
    } else if (is.integer(sha_2021$CERT_TYPE) & min(act_types %in% 1:15) == 0) {
      qa_act_note <- glue("The following unexpected action types were present: ",
    ...
    prog_types <- sort(unique(sha_2021$PROGRAM_TYPE[!is.na(sha_2021$PROGRAM_TYPE)]))
    ...
    DBI::dbExecute(conn,
                   glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                            (etl_batch_id, last_run, table_name, 
                              qa_type, qa_item, qa_result, qa_date, note) 
                            VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                    'value', 'household_count', {length(unique(sha_2021$HH_CERT_ID))},
                                    {Sys.time()}, 'HCV and PH both included')",
                            .con = conn))
  ...
  # RENAME FIELDS ----
    ...
    if(!'vouch_type' %in% names(sha_2021)){
  ...
  # DATA CLEANING ----
    ## Replace "NULL" with true NA ----
      for(nombre in names(sha_2021)[sapply(sha_2021, is.character)]){
        sha_2021[get(nombre) == 'NULL', paste0(nombre) := NA_character_]
      }

    ## Deduplicate data to avoid extra rows when joining ----
      sha_2021 <- unique(sha_2021)
  
  
    ## Fix up date formats ----
      for(datevar in c('dob', grep('_date$', names(sha_2021), value = T))){
        sha_2021[class(get(datevar)) != 'Date', paste0(datevar) := as.Date(as.integer(get(datevar)), origin = '1899-12-30')]
      }

    ## Income ----
      # 1) Reshape income from wide to long
        sha_2021_inc <- melt(sha_2021, 
                             id.vars = c('cert_id', 'act_date', 'ssn'), 
                             measure.vars = grep("^inc_", names(sha_2021), value = T), 
                             value.name = 'inc',
                             variable.name = 'inc_code', 
                             na.rm = T)
        sha_2021_inc[, inc_code := gsub('inc_', '', inc_code)]    
      # 2) Identify people with income from a fixed source
        sha_2021_inc[, inc_fixed := 0][tolower(inc_code) %in% c("p", "pension", "s", "ssi", "ss", "social security"), inc_fixed := 1]
      # 3) Summarize income/assets for a given time point to reduce duplicated rows
        sha_2021_inc <- sha_2021_inc[, .(inc = sum(inc, na.rm = T), inc_fixed = min(inc_fixed, na.rm = T)), .(cert_id, act_date, ssn)]
        sha_2021_inc[, hh_inc_calc := sum(inc, na.rm = T), cert_id]
        sha_2021_inc[, hh_inc_fixed := min(inc_fixed, na.rm = T), cert_id]
  ...
  # COMBINE DATA ----
    ## Join/Merge main data with income data ----
      # Using a left_join because without the panel 1 info (names, SSN, etc.) the info is not much help
        sha_2021 <- merge(sha_2021, 
                            sha_2021_inc[, .(cert_id, act_date, ssn, inc, inc_fixed)], 
                            by = c("cert_id", "act_date", "ssn"), 
                            all.x = T, all.y = F)
      # Join hh-level income info separately to avoid NAs on hh members who don't appear in panel 2
        sha_2021 <- merge(sha_2021, 
                      unique(sha_2021_inc[, .(cert_id, act_date, hh_inc_calc, hh_inc_fixed)]), 
    ...
    ## Join/Merge main data with portfolio data ----
      sha_2021 <- merge(sha_2021, 
                        ...
                        all.x = T, all.y = F)    
      sha_2021[, prog_type := prog_type.x][!is.na(prog_type.y), prog_type := prog_type.y]
      sha_2021[, property_id := property_id.x][!is.na(property_id.y), property_id := property_id.y]
      sha_2021[, c('prog_type.x', 'prog_type.y', 'property_id.x', 'property_id.y') := NULL]
        
  # LOAD DATA TO SQL ----
  # Add source field to track where each row came from
  sha_2021 <- sha_2021 %>% 
    mutate(pha_source = "sha2021",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_2021),
