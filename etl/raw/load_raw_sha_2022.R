#### CODE TO LOAD 2022 SEATTLE HOUSING AUTHORITY PUBLIC HOUSING AND VOUCHER DATA
# Alastair Matheson, PHSKC (APDE)
# 2021-06
# Revised by Danny Colombara, 2023-03-15 due to new data
...
load_raw_sha_2022 <- function(conn = NULL,
                              to_schema = NULL,
                              to_table = NULL,
                              qa_schema = NULL,
                              qa_table = NULL,
  ...
  # BRING IN DATA ----
  sha_2022 <- fread(file = file.path(file_path, "sha_hcv_ph_2022.csv"), 
  ...
  for(incvar in grep('income', names(sha_2022), value = T, ignore.case = T)){
    sha_2022[, paste0(incvar) := as.integer(get(gsub(",", "", incvar)))]
  }
  ...
  # QA CHECKS ----
    ...
    sha_2022[class(EFFECTIVE_DATE) != 'Date', EFFECTIVE_DATE := as.Date(as.integer(EFFECTIVE_DATE), origin = '1899-12-30')]
    dates <- sha_2022 %>%
    ...
    act_types <- sort(unique(sha_2022$CERT_TYPE[!is.na(sha_2022$CERT_TYPE)]))
    ...
    if (is.character(sha_2022$CERT_TYPE) & length(act_types[act_types %in% act_types_expected == F]) > 0) {
    ...
    } else if (is.integer(sha_2022$CERT_TYPE) & min(act_types %in% 1:15) == 0) {
      qa_act_note <- glue("The following unexpected action types were present: ",
    ...
    prog_types <- sort(unique(sha_2022$PROGRAM_TYPE[!is.na(sha_2022$PROGRAM_TYPE)]))
  ...
  # CHECK QA PASSED ----
    ...
    if (min(qa_names_result, qa_date_result, qa_act_result, qa_prog_result, qa_portfolio_result) == "FAIL") {
      ...
      rm(list = setdiff(ls(pattern = "^qa_"), c('qa_schema', 'qa_table')))
      rm(namez)
      rm(dates)
  ...
  # DEDUPLICATION ----
    ...
    for(nombre in names(sha_2022)[sapply(sha_2022, is.character)]){
      sha_2022[get(nombre) == 'NULL', paste0(nombre) := NA_character_]
    }
    
    ## Deduplicate data to avoid extra rows when joining
    sha_2022 <- unique(sha_2022)
    ...
    DBI::dbExecute(conn,
                   glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                            (etl_batch_id, last_run, table_name, 
                              qa_type, qa_item, qa_result, qa_date, note) 
                            VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                    'value', 'household_count', {length(unique(sha_2022$HH_CERT_ID))},
                                    {Sys.time()}, 'HCV and PH both included')",
                            .con = conn))
  ...
  # RENAME FIELDS ----
    ...
    if(!'vouch_type' %in% names(sha_2022)){
  ...
  # DATA CLEANING ----
    ## Fix up date formats ----
      for(datevar in c('dob', grep('_date$', names(sha_2022), value = T))){
        sha_2022[class(get(datevar)) != 'Date', paste0(datevar) := as.Date(as.integer(get(datevar)), origin = '1899-12-30')]
      }

    ## Income ----
      # 1) Reshape income from wide to long
        sha_2022_inc <- melt(sha_2022, 
                             id.vars = c('cert_id', 'act_date', 'ssn'), 
                             measure.vars = grep("^inc_", names(sha_2022), value = T), 
                             value.name = 'inc',
                             variable.name = 'inc_code', 
                             na.rm = T)
        sha_2022_inc[, inc_code := gsub('inc_', '', inc_code)]    
      # 2) Identify people with income from a fixed source
        sha_2022_inc[, inc_fixed := 0][tolower(inc_code) %in% c("p", "pension", "s", "ssi", "ss", "social security"), inc_fixed := 1]
      # 3) Summarize income/assets for a given time point to reduce duplicated rows
        sha_2022_inc <- sha_2022_inc[, .(inc = sum(inc, na.rm = T), inc_fixed = min(inc_fixed, na.rm = T)), .(cert_id, act_date, ssn)]
        sha_2022_inc[, hh_inc_calc := sum(inc, na.rm = T), cert_id]
        sha_2022_inc[, hh_inc_fixed := min(inc_fixed, na.rm = T), cert_id]
  ...
  # COMBINE DATA ----
    ## Join/Merge main data with income data ----
      # Using a left_join because without the panel 1 info (names, SSN, etc.) the info is not much help
        sha_2022 <- merge(sha_2022, 
                            sha_2022_inc[, .(cert_id, act_date, ssn, inc, inc_fixed)], 
                            by = c("cert_id", "act_date", "ssn"), 
                            all.x = T, all.y = F)
      # Join hh-level income info separately to avoid NAs on hh members who don't appear in panel 2
        sha_2022 <- merge(sha_2022, 
                      unique(sha_2022_inc[, .(cert_id, act_date, hh_inc_calc, hh_inc_fixed)]), 
    ...
    ## Join/Merge main data with portfolio data ----
      sha_2022 <- merge(sha_2022, 
                        ...
                        all.x = T, all.y = F)    
      sha_2022[, prog_type := prog_type.x][!is.na(prog_type.y), prog_type := prog_type.y]
      sha_2022[, property_id := property_id.x][!is.na(property_id.y), property_id := property_id.y]
      sha_2022[, c('prog_type.x', 'prog_type.y', 'property_id.x', 'property_id.y') := NULL]
        
  # LOAD DATA TO SQL ----
  # Add source field to track where each row came from
  sha_2022 <- sha_2022 %>% 
    mutate(pha_source = "sha2022",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_2022),
