#### CODE TO LOAD 2019 SEATTLE HOUSING AUTHORITY VOUCHER DATA
...
load_raw_sha_hcv_2020 <- function(conn = NULL,
                              to_schema = NULL,
                              to_table = NULL,
                              qa_schema = NULL,
                              qa_table = NULL,
  ...
  # BRING IN DATA ----
  sha_hcv_2020 <- fread(file = file.path(file_path, "sha_hcv_2020.csv"), 
  ...
  act_types <- sort(unique(sha_hcv_2020$CERT_TYPE[!is.na(sha_hcv_2020$CERT_TYPE)]))
  ...
  if (is.character(sha_hcv_2020$CERT_TYPE) & length(act_types[act_types %in% act_types_expected == F]) > 0) {
    ...
    qa_act_result <- "FAIL"
    warning(qa_act_note)
  } else if (is.integer(sha_hcv_2020$CERT_TYPE) & min(act_types %in% c(1:15, 99)) == 0) {
    qa_act_note <- glue("The following unexpected action types were present: ",
  ...
  prog_types <- sort(unique(sha_hcv_2020$PROGRAM_TYPE[!is.na(sha_hcv_2020$PROGRAM_TYPE)]))
  ...
  portfolio_impact <- inner_join(portfolio_miss, 
                                 select(sha_hcv_2020, BUILDING_ID) %>% 
                                   mutate(BUILDING_ID = as.character(BUILDING_ID)))
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                  'value', 'row_count', {nrow(sha_hcv_2020)},
                                  {Sys.time()}, NULL)",
                          .con = conn))
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          (etl_batch_id, last_run, table_name, qa_type, qa_item, qa_result, qa_date, note) 
                          VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                  'value', 'household_count', {length(unique(sha_hcv_2020$HH_CERT_ID))},
                                  {Sys.time()}, NULL)",
                          .con = conn))
  ...
  sha_hcv_2020 <- left_join(sha_hcv_2020, 
                        select(sha_hcv_2020_inc, cert_id, act_date, ssn, starts_with("inc")) %>% distinct(), 
                        by = c("cert_id", "act_date", "ssn")) %>%
    left_join(., select(sha_hcv_2020_inc, cert_id, act_date, starts_with("hh_")) %>% distinct(),
  ...
  sha_hcv_2020 <- sha_hcv_2020 %>% 
    mutate(pha_source = "sha2020_hcv",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_hcv_2020),
