#### CODE TO LOAD 2004-2006 SEATTLE HOUSING AUTHORITY PUBLIC HOUSING DATA
...
load_raw_sha_ph_2004_2006 <- function(conn = NULL,
                                    to_schema = NULL,
                                    to_table = NULL,
                                    qa_schema = NULL,
                                    qa_table = NULL,
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                  'value', 'row_count', {nrow(sha_ph_p1_2004_2006)},
  ...
  sha_ph_2004_2006 <- list(sha_ph_p1_2004_2006 = sha_ph_p1_2004_2006, 
                           sha_ph_p2_2004_2006 = sha_ph_p2_2004_2006, 
                           sha_ph_p3_2004_2006 = sha_ph_p3_2004_2006)
  ...
  sha_ph_2004_2006 <- sha_ph_2004_2006 %>%
    map(~ .x %>% 
  ...
  sha_ph_2004_2006 <- sha_ph_2004_2006 %>% map(~ .x %>% distinct())
  
  # Bring back to individual objects to make using data table easier
  list2env(sha_ph_2004_2006, envir = environment())
  rm(sha_ph_2004_2006)
  
  
  ## Panel 1 ----
  # Fix up date formats
  sha_ph_p1_2004_2006 <- sha_ph_p1_2004_2006 %>%
  ...
  sha_ph_2004_2006 <- left_join(sha_ph_p1_2004_2006, 
                             select(sha_ph_p2_2004_2006, starts_with("inc")) %>% distinct(), 
                             by = c("incasset_id", "mbr_num" = "inc_mbr_num")) %>%
    left_join(., select(sha_ph_p2_2004_2006, incasset_id, starts_with("hh_")) %>% distinct(),
              by = "incasset_id") %>%
    left_join(., sha_ph_p3_2004_2006, by = "incasset_id")
  
  
  ## Join with portfolio data ----
  sha_ph_2004_2006 <- left_join(sha_ph_2004_2006, 
  ...
  sha_ph_2004_2006 <- sha_ph_2004_2006 %>% 
    mutate(pha_source = "sha2006_ph",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_ph_2004_2006),
