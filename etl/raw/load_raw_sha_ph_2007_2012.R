#### CODE TO LOAD 2007-2012 SEATTLE HOUSING AUTHORITY PUBLIC HOUSING DATA
...
load_raw.sha_ph_2007_2012 <- function(conn = NULL,
  ...
  # BRING IN DATA ----
  sha_ph_p1_2007_2012 <- fread(file = file.path(file_path, "sha_ph_2007_2012_panel_01_household_suffix_corrected.csv"), 
                             na.strings = c("NA", "", "NULL", "N/A", "."), 
                             stringsAsFactors = F)
  sha_ph_p2_2007_2012 <- fread(file = file.path(file_path, "sha_ph_2007_2012_panel_02_income_suffix_corrected.csv"), 
                             na.strings = c("NA", "", "NULL", "N/A", "."), 
                             stringsAsFactors = F)
  sha_ph_p3_2007_2012 <- fread(file = file.path(file_path, "sha_ph_2007_2012_panel_03_assets_suffix_corrected.csv"), 
                             na.strings = c("NA", "", "NULL", "N/A", "."), 
                             stringsAsFactors = F)
  ...
  DBI::dbExecute(conn,
                 glue_sql("INSERT INTO {`qa_schema`}.{`qa_table`} 
                          ...
                          VALUES ({etl_batch_id}, NULL, '{DBI::SQL(to_schema)}.{DBI::SQL(to_table)}',
                                  'value', 'row_count', {nrow(sha_ph_p1_2007_2012)},
  ...
  sha_ph_2007_2012 <- list(sha_ph_p1_2007_2012 = sha_ph_p1_2007_2012, 
                           sha_ph_p2_2007_2012 = sha_ph_p2_2007_2012, 
                           sha_ph_p3_2007_2012 = sha_ph_p3_2007_2012)
  ...
  sha_ph_2007_2012 <- sha_ph_2007_2012 %>%
    map(~ .x %>% 
  ...
  sha_ph_2007_2012 <- sha_ph_2007_2012 %>% map(~ .x %>% distinct())
  
  # Bring back to individual objects to make using data table easier
  list2env(sha_ph_2007_2012, envir = environment())
  rm(sha_ph_2007_2012)
  
  
  ## Panel 1 ----
  # Fix up date formats
  sha_ph_p1_2007_2012 <- sha_ph_p1_2007_2012 %>%
  ...
  sha_ph_2007_2012 <- left_join(sha_ph_p1_2007_2012, 
                             select(sha_ph_p2_2007_2012, starts_with("inc")) %>% distinct(), 
  ...
  sha_ph_2007_2012 <- sha_ph_2007_2012 %>% 
    mutate(pha_source = "sha2012_ph",
           etl_batch_id = etl_batch_id)
  
  # Load data
  dbWriteTable(conn,
               name = DBI::Id(schema = to_schema, table = to_table),
               value = as.data.frame(sha_ph_2007_2012),
