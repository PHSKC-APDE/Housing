...
##
## Purpose of script: Set up PHA exit data for the HUD HEARS Study
##
...
pacman::p_load(tidyverse, odbc, glue, data.table, readxl)

pha_path <- "//phdata01/DROF_DATA/DOH DATA/Housing"
...
# INITIAL CLEAN UP ----
### Some tidying is best done separately
## Dates ----
kcha_exit <- kcha_exit %>%
  map(~ .x %>%
...
## Source ----
kcha_exit <- kcha_exit %>% map(~ .x %>% mutate(pha_source = "kcha_exit"))

sha_exit <- sha_exit %>% map(~ .x %>% mutate(pha_source = "sha_exit"))


# COMBINE AND CLEAN DATA ----
pha_exit = c(kcha_exit, sha_exit)
    ...
    .x %>% 
      ...
      mutate(geo_hash_raw = as.character(toupper(openssl::sha256(
        paste(str_replace_na(geo_add1_raw, ''),
              ...
              if ("geo_state_raw" %in% names(.x)) {str_replace_na(geo_state_raw, '')} else {'WA'},
...
DBI::dbExecute(conn,
               glue_sql("INSERT INTO pha.metadata_qa 
                          ...
                          VALUES (NULL, {last_run}, 'pha.stage_exit',
...
lapply(seq(start, cycles), function(i) {
  ...
  if (i == 1) {
    dbWriteTable(db_hhsaw,
                 name = DBI::Id(schema = "pha", table = "stage_exit"),
