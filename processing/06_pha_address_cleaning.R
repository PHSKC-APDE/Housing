...
if (UW == F & sql == T) {
  ...
  # process below
  adds_specific <- dbGetQuery(db_claims,
                              ...
                              geo_state_clean, geo_zip_clean, geo_source_pha
                              FROM ref.address_clean
                              WHERE geo_source_pha = 1")
  
  
  # Convert NULLs to NA so they join properly
  # ZIPs are stored as character in ref table so need to convert pha data
  # Eventually move this earlier in the processing
  pha_cleanadd <- pha_cleanadd %>%
    mutate_at(vars(unit_add, unit_apt, unit_apt2, unit_city, unit_state, unit_zip),
              funs(ifelse(. == "" | . == "NA", NA_character_, .)))
  pha_cleanadd <- pha_cleanadd %>%
    left_join(., adds_specific, 
              by = c("unit_add" = "geo_add1_raw",
                     "unit_apt" = "geo_add2_raw",
                     "unit_apt2" = "geo_add3_raw",
  ...
  # Check how many matched (overall and after removing blank addresses)
  pha_cleanadd %>% group_by(geo_source_pha) %>% summarise(count = n())
  pha_cleanadd %>% 
    filter(!(is.na(unit_add) & is.na(unit_apt) & is.na(unit_apt2) & 
               is.na(unit_city) & is.na(unit_state) & is.na(unit_zip))) %>%
    group_by(geo_source_pha) %>% summarise(count = n())
  
  
  # Create df of new addresses that need cleaning
  address_new <- pha_cleanadd %>%
    filter(is.na(geo_source_pha) & 
             !(is.na(unit_add) & is.na(unit_apt) & is.na(unit_apt2) & 
                 is.na(unit_city) & is.na(unit_state) & is.na(unit_zip)))
...
if (UW == FALSE) {
  if (nrow(address_new) > 0) {
    
    # Set up columns to match what is expected by the cleaning function
    address_new <- address_new %>%
      mutate(unit_add_new = unit_add,
             unit_apt_new = unit_apt,
             unit_apt2_new = unit_apt2,
             unit_city_new = unit_city,
             unit_state_new = unit_state,
             unit_zip_new = unit_zip)
    
    # Run new addresses through the cleaning function
    address_new <- add_clean_f(address_new)
    
    ### Send new addresses to SQL where they will eventually be cleaned via Informatica
    # Pull out distinct new addresses and rename to match PHSKC address tables
    # Note that the 'clean' addresses are still called raw for the purposes of
    #    additional cleaning. They are an intermediary step between the actual
    #    raw addresses and a final clean version.
    
    address_new_load <- address_new %>%
      distinct(unit_add, unit_apt, unit_apt2, unit_city, unit_state, unit_zip,
        unit_add_new, unit_apt_new, unit_city_new, unit_state_new, unit_zip_new) %>%
      rename(geo_add1_raw = unit_add_new,
             geo_add2_raw = unit_apt_new,
             geo_city_raw = unit_city_new,
             geo_state_raw = unit_state_new,
             geo_zip_raw = unit_zip_new) %>%
      mutate(geo_source_pha = 1)
    
    tbl_id_meta <- DBI::Id(schema = "stage", table = "pha_address_new")
    
    if (dbExistsTable(db_apde51, tbl_id_meta)) {
      dbRemoveTable(db_apde51, tbl_id_meta)
    }
    
    dbWriteTable(db_apde51, tbl_id_meta, address_new_load, overwrite = T)
    rm(tbl_id_meta)
  }
  
  
  ### Combine already cleaned or blank rows with newly cleaned ones
  pha_cleanadd <- pha_cleanadd %>%
    filter(!is.na(geo_source_pha) |
             (is.na(unit_add) & is.na(unit_apt) & is.na(unit_apt2) & 
                is.na(unit_city) & is.na(unit_state) & is.na(unit_zip))) %>%
    bind_rows(., address_new)
}
...
pha_cleanadd <- pha_cleanadd %>% arrange(pid, act_date, agency_prog_concat) %>%
  mutate_at(vars(unit_add_new, unit_apt_new, unit_city_new, unit_state_new, unit_zip_new),
            funs(ifelse((. == "" | is.na(.) | . == "0") & pid == lag(pid, 1) & !is.na(lag(pid, 1)) & 
