path = '//home/joseh/source/Housing/processing/'

source(paste0(path,'01_kcha_sql_load.R'))
source(paste0(path,'02_sha_sql_load.R'))
source(paste0(path,'03_pha_combining.R'))
source(paste0(path,'04_pha_matching.R'))
source(paste0(path,'05_pha_recodes.R'))
source(paste0(path,'06_pha_address_cleaning.R'))
source(paste0(path,'06a_pha_geocoding.R'))
source(paste0(path,'07_pha_consolidation.R'))
source(paste0(path,'08_pha_analyses_prep.R'))

gc()

pha_compare <- fread(file = "//home/joseh/data/HILD/pha_longitudinal.csv", 
                   na.strings = c("NA", "", "NULL", "N/A"),
                   stringsAsFactors = F)

pha_compare_old <-  fread(file = "//home/joseh/data/HILD/pha_longitudinal_09_18.csv", 
                          na.strings = c("NA", "", "NULL", "N/A"),
                          stringsAsFactors = F)
...
if (UW == TRUE) {
  ...
  drop_track <- left_join(drop_track, drop_temp, by = "row") } else {
...
if (UW == TRUE) {
  print("Keep droptype 4") 
  dfsize_head <- nrow(pha_cleanadd_sort)
  pha_cleanadd_sort <- pha_cleanadd_sort %>%
    mutate(
      # Start with non-port-out rows with act_type == 10 or 16)
      drop_check = ifelse(unit_concat %in% c(",,,,NA", ",,,,0") & 
                            (act_type == 10 | 
                               (act_type == 16 &  
                                  !(port_out_kcha == 1 | port_out_sha == 1))), 3, drop_check),
      # Also drop people who were only at KCHA and only had missing addresses
      drop_check = ifelse(agency_new == "KCHA" & add_num == 1 & 
...
if (UW == TRUE) {
  print("Keep droptype 5") 
  dfsize_head <- nrow(pha_cleanadd_sort)
  pha_cleanadd_sort <- pha_cleanadd_sort %>%
    arrange(pid, act_date, agency_prog_concat, unit_concat, cost_pha) %>%
    mutate(drop_check = if_else(pid == lead(pid, 1) & !is.na(lead(pid, 1)) & 
                                  act_date == lead(act_date, 1) &
                                  agency_prog_concat == lead(agency_prog_concat, 1) & 
                                  unit_concat == lead(unit_concat, 1) &
                                  cost_pha == "" & lead(cost_pha != ""), 5, drop_check)) 
